...
const WeeklyCalendarGrid = () => {
  ...
  const hasEvent = (day, hour, quarter) =>
    events.some(event => event.day === day && event.hour === hour && event.quarter === quarter);
        ...
        <thead>
          <tr>
            ...
            {daysOfWeek.map((day, dayIndex) => (
              <th key={dayIndex} style={{ width: `${90 / daysOfWeek.length}%` }}>
                {day}
              </th>
            ))}
          </tr>
            ...
            return (
              <tr key={timeIndex}>
                <td className="hour-label">
                  {`${hour}:${quarter === 0 ? '00' : quarter}`} {/* Display quarter-hour intervals */}
                </td>
                {daysOfWeek.map((day, dayIndex) => (
                  <td
                    key={dayIndex}
                    className={`calendar-cell ${hasEvent(day, hour, timeSlot % 4) ? 'has-event' : ''}`}
                  >
                    {/* Render events or other content here */}
                  </td>
                ))}
              </tr>
            );
